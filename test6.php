...
if($_COOKIE['length']&& isset($_POST['yes'])){
  $abc=$_COOKIE['length'];
...
if(isset($_POST['confirm'])){
  ...
  if(in_array($worksextension,$allowedfileExtensions)){
    $newworksname=(sizeof($workfiles)).($ut).($id).($worksname);
    $uploadworkDir='./sample_files/';
    $workdest_path=$newworksname;
    move_uploaded_file($works,$uploadworkDir.$workdest_path);
...
<video style="position:relative; z-index:1;" width="320px" height="240px" autoplay muted onmouseover="displaycancel($i)" onmouseout="hidecancel($i)">
<source src="./sample_files/$workfiles[$i]" type="video/mp4">
</video>
<img class='cancel' id="$i" style="position:absolute; z-index:2; bottom:110px; margin-left:295px; display: none;" onmouseover="displaycancel($i)" onmouseout="hidecancel($i)" onclick="remove($i)" src='resources/cancel3.png' width='20px'>
<div class="prompt" id="$a">
<center>
<p style="color:white;">Are you sure you want to delete this?</p>
<input type="button" value="Add video" onclick="uploadvideo()" id="videobtn">
